(() => {
  ...
  function buildInstallSnippet(key = "YOUR_PAIRING_KEY") {
    ...
    function windowsAgentBootstrapFromRepoDir(srvRaw, pairingKeyRaw) {
      ...
      return `$mgr=(Get-Location).Path; Set-Location $mgr; Write-Host 'npm install (keeps deps right after git pull)...' -ForegroundColor Cyan; npm install; if ($LASTEXITCODE -ne 0) { Write-Host 'npm install failed.' -ForegroundColor Red; exit 1 }; $srv='${srv}'; $pairkey='${pairKey}'; $bLog=(Join-Path $env:USERPROFILE '.aj-server-manager-agent-boot.log'); Write-Host ('Agent writes each run to: ' + $bLog + ' — open in Notepad if this PC stays Offline.') -ForegroundColor Gray; $node=(Get-Command node).Source; $script=(Resolve-Path (Join-Path $mgr 'src/agent-cli.js')).Path; Write-Host ('Starting agent (repo: {0}) → dashboard {1} ...' -f $mgr,$srv) -ForegroundColor Cyan; Start-Process -WindowStyle Hidden -FilePath $node -WorkingDirectory $mgr -ArgumentList @($script,'--server',$srv,'--key',$pairkey); try { $who=[System.Security.Principal.WindowsIdentity]::GetCurrent().Name; $argLine=('"{0}" --server "{1}"' -f $script,$srv); $stAction=New-ScheduledTaskAction -Execute $node -WorkingDirectory $mgr -Argument $argLine; $stTrigger=New-ScheduledTaskTrigger -AtLogOn -User $who; $stPrincipal=New-ScheduledTaskPrincipal -UserId $who -LogonType Interactive -RunLevel Limited; Register-ScheduledTask -TaskName 'aj-server-manager' -Action $stAction -Trigger $stTrigger -Principal $stPrincipal -Force | Out-Null } catch { Write-Host ('Note: Could not register log-on task ({0}). Agent was started without it; run this script in PowerShell as Administrator for auto-start at sign-in.' -f $_.Exception.Message) -ForegroundColor DarkYellow }; Write-Host 'If Offline after ~25s: read boot log above, firewall on dashboard PC port 3847, new pairing key.' -ForegroundColor Gray`;
    }
